...
var collColor = document.getElementsByClassName("filter-color-name");
...
for(let i=0; i<collColor.length;i++){
    collColor[i].addEventListener("click",function(){
        this.classList.toggle("active");
        let content = this.nextElementSibling;
        if(content.classList.contains("color-category")){
            if(content.style.maxHeight){
                content.style.maxHeight = null;
                content.style.display = "none";
            } else {
